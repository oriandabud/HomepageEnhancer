  ...
  class ViewAble
    ...
    def initialize(url , website)
      ...
      if doc.at_css(website.product_page.validator_selector)
        ...
        [Product.create(page_link: url,picture_link: picture_link, title: title , price: price , website: website)]


      elsif doc.at_css(website.category_page.validator_selector)

        Category.create(page_link: url, website: website)

        products_urls = doc.at_css(website.category_page.products_selector).xpath('@href')

        products = []
        products_urls.each do |product_url|
          products += ViewAble.new(product_url,website)
        end
        products
